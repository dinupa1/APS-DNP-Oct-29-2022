...
\item In the SpinQuest experiment \jpsi production should be dominated by the $q\bar{q}$ annihilation \citeme{STAR:2021tve}.

\item Our goal is to measure $A_{N}$ with an absolute error $\mathcal{O}(10^{-2})$ for a few $p_{T}$ and/or $x_{F}$ bins.

\item In this presentation, we demonstrate the analysis procedure and  extraction of single spin asymmetry ($A_{N}$)
for a few $p_{T}$ and $x_{F}$ bins.
...
\item We fit this kernel in side-band regions on either side of the \jpsi invariant mass peak. Then we used the trained
kernel  to predict the background in the \jpsi peak region.
...
\begin{figure}
    ...
    \caption{Mass histogram for 1st $p_{T}$ bin and 1st $\phi$ bin. Predicted background is given in shaded red region.
    Side-bands are indicated in dashed blue lines.}
\end{figure}
\end{textblock}
...
\begin{figure}
    ...
    \caption{GPR prediction for background.}
\end{figure}
\end{textblock}
...
\begin{itemize}
  ...
  \item As shown in figure 9, the prediction from GPR method agrees with the Drell-Yan events with 95\% confidence interval
  in the \jpsi mass region.
